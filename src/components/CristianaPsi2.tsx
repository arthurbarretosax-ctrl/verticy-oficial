...
import "@/assets/css/cristianapsi2.css";

export default function CristianaPsi2({ content }: { content: any }) {
  return (
    <div className="cristianapsi2-wrapper template-cristiana psi2">
      <div className="relative overflow-x-hidden selection:bg-psi-accent selection:text-white pb-0">
  ...
  <div className="max-w-[450px] mx-auto min-h-screen relative bg-psi-black shadow-2xl flex flex-col sm:border-x sm:border-white/5 overflow-hidden">
    ...
    <header className="relative w-full h-[95vh] overflow-hidden flex flex-col items-center">
      ...
      <div className="absolute top-6 left-6 z-50">
        <div className="w-14 h-14 rounded-full bg-psi-black/40 backdrop-blur-md border border-white/10 shadow-lg overflow-hidden flex items-center justify-center">
           <img id="img-logo" src={content.img_1} alt="Logo" className="w-full h-full object-cover rounded-full"  />
        </div>
      </div>
      ...
      <div className="absolute bottom-40 left-0 w-full text-center z-10 pointer-events-none px-4 flex flex-col items-center">
        <h1 className="text-5xl font-serif font-bold text-white leading-tight tracking-tight drop-shadow-[0_5px_15px_rgba(0,0,0,0.8)]">{content.text_1}<br />{content.text_2}</h1>
        <p className="text-[9px] font-bold text-psi-accent uppercase tracking-[0.5em] mt-5 italic drop-shadow-md">{content.text_3}</p>
      </div>

      
      <div className="absolute bottom-16 left-0 w-full px-10 z-20 reveal active">
      </div>
    ...
    <div className="w-full overflow-hidden border-y border-white/5 py-4 relative z-30 bg-psi-dark/80 backdrop-blur-md">
      <div className="animate-marquee flex gap-12 items-center text-white/30 italic font-medium">
        <span className="text-[11px] font-bold uppercase tracking-[0.4em]">{content.text_4}</span>
    ...
    <section className="px-6 py-20 reveal">
      <div className="text-center mb-12 px-4">
        <h3 className="text-[10px] font-bold text-psi-accent uppercase tracking-[0.5em] mb-4">{content.text_8}</h3>
        <h2 className="font-serif text-4xl text-white italic">{content.text_9}<br /><span className="text-white not-italic font-black uppercase tracking-tighter">{content.text_10}</span></h2>
      </div>
      ...
      <div id="quiz-container" className="glass-premium p-10 rounded-[3rem] relative min-h-[420px] flex flex-col justify-center overflow-hidden">
        <div id="quiz-start" className="text-center">
          ...
          <p className="text-sm text-slate-400 mb-10 leading-relaxed font-medium">{content.text_11}</p>
          <button  className="w-full border border-psi-accent text-psi-accent py-4 rounded-2xl font-bold text-[11px] uppercase tracking-[0.3em] hover:bg-psi-accent hover:text-white transition-all active:scale-95">{content.text_12}</button>
        </div>

        <div id="quiz-steps" className="hidden flex-col h-full">
           <div className="mb-10">
              <span id="quiz-progress" className="text-[10px] font-black text-psi-accent uppercase tracking-widest">{content.text_13}</span>
              <div className="w-full h-1 bg-white/5 rounded-full mt-2 overflow-hidden">
                <div id="quiz-bar" className="w-[33%] h-full bg-psi-accent transition-all duration-500"></div>
              </div>
           </div>
           <h4 id="quiz-question" className="font-serif text-2xl text-white mb-10 leading-tight">{content.text_14}</h4>
           <div className="space-y-3" id="quiz-options">
              <button  className="w-full border border-white/10 p-5 rounded-2xl text-xs font-medium text-left flex items-center justify-between group active:bg-psi-accent transition-all">{content.text_15}<i data-lucide="chevron-right" className="w-4 h-4 opacity-30"></i></button>
        ...
        <div id="quiz-result" className="hidden text-center py-6">
           ...
           <h3 className="font-serif text-3xl text-white mb-4">{content.text_18}</h3>
           <p className="text-sm text-slate-400 leading-relaxed mb-10 font-medium">{content.text_19}</p>
           <button  className="w-full bg-white text-psi-black py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-xl active:scale-95 transition-all">{content.text_20}</button>
        </div>
      </div>
    </section>
    ...
    <section className="px-6 py-12 reveal">
      <div className="glass-premium rounded-[3.5rem] overflow-hidden border border-white/5 shadow-2xl relative group bg-gradient-to-br from-psi-dark to-psi-black">
        ...
        <div className="p-10 pt-8 relative z-10 text-center mx-auto">
          <h2 className="font-serif text-4xl font-black text-white italic uppercase tracking-tighter leading-none mb-6">{content.text_21}<br /><span className="text-psi-accent">{content.text_22}</span></h2>
          <div className="space-y-6 text-sm text-slate-400 leading-relaxed font-medium">
            <p>{content.text_23}</p>
            <p>{content.text_24}<strong className="text-white">{content.text_25}</strong>{content.text_26}</p>
          </div>
          <div className="mt-10 pt-8 border-t border-white/5">
             <div className="flex flex-wrap justify-center gap-2">
                <span className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[9px] font-black uppercase tracking-widest text-psi-accent italic">{content.text_27}</span>
                <span className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[9px] font-black uppercase tracking-widest text-psi-accent italic">{content.text_28}&{content.text_29}</span>
             </div>
          </div>
        </div>
      </div>
    ...
    <section className="px-6 py-20 bg-psi-dark/20 border-y border-white/5 reveal">
      <div className="text-center mb-16">
        <h2 className="font-display text-2xl uppercase tracking-widest leading-none">{content.text_30}<br /><span className="text-psi-accent italic">{content.text_31}</span></h2>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="glass-premium p-6 rounded-[2.5rem] flex flex-col justify-between h-44 relative overflow-hidden">
           ...
           <div>
             <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-1">{content.text_32}</h4>
             <p className="text-[10px] text-slate-500 leading-tight">{content.text_33}</p>
           </div>
        </div>
        <div className="glass-premium p-6 rounded-[2.5rem] flex flex-col justify-between h-44 border-t-2 border-t-psi-accent/20">
           ...
           <div>
             <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-1">{content.text_34}</h4>
             <p className="text-[10px] text-slate-500 leading-tight">{content.text_35}</p>
           </div>
        </div>
        <div className="glass-premium p-6 rounded-[2.5rem] col-span-2 flex items-center gap-6 border-l-4 border-l-psi-accent">
           ...
           <div>
             <h4 className="text-sm font-bold text-white uppercase tracking-widest">{content.text_36}</h4>
             <p className="text-[11px] text-slate-400">{content.text_37}</p>
           </div>
        </div>
      </div>
    </section>
    ...
    <section className="px-10 py-32 text-center reveal relative overflow-hidden">
       ...
       <h3 className="font-serif text-[2.2rem] text-white leading-tight italic drop-shadow-xl relative z-10">{content.text_38}<br /><span className="text-psi-accent not-italic font-black uppercase tracking-tighter text-3xl">{content.text_39}</span>
       </h3>
       <p className="text-[10px] font-black uppercase tracking-[0.5em] text-psi-accent mt-12 opacity-60">{content.text_40}</p>
    </section>

    
    <footer className="mt-auto px-8 pt-16 pb-24 bg-psi-dark/40 border-t border-white/5 text-center reveal">
      <h3 className="font-display text-2xl mb-12 tracking-tighter uppercase leading-none italic text-center mx-auto">{content.text_41}<br /><span className="text-psi-accent opacity-60 font-black">{content.text_42}</span></h3>
      <div className="space-y-4 max-w-xs mx-auto mb-16">
        <button  className="w-full bg-white text-psi-black py-5 rounded-full font-black text-xs uppercase tracking-widest shadow-xl active:scale-[0.97] transition-all flex items-center justify-center gap-3">
          <i data-lucide="message-circle" className="w-5 h-5"></i>{content.text_43}</button>
        <button  className="w-full glass-premium border border-white/10 text-white py-5 rounded-full font-bold text-[11px] uppercase tracking-widest flex items-center justify-center gap-3 active:bg-white/10 transition-all">
          <i data-lucide="instagram" className="w-5 h-5 text-pink-500"></i>{content.text_44}</button>
      </div>
      <div className="mt-24 pt-10 border-t border-white/5 opacity-10 text-center mx-auto">
        <p className="text-[8px] font-bold uppercase tracking-[0.5em] mb-4">{content.text_45}</p>
        <p className="text-[8px] uppercase tracking-[0.4em]">{content.text_46}<strong className="text-white tracking-[0.2em] font-black uppercase text-center mx-auto">{content.text_47}</strong>{content.text_48}</p>
      </div>
    </footer>
